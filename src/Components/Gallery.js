            ...
            <div className={`gallery`}>
                {datas.sort(function () {
                    ...
                    <div className="gallery-item" key={index} onClick={() => getImg(pic.img, pic?.title)}>
                        ...
                        transition ease-in-out delay-75 hover:-translate-y-1 hover:scale-110 duration-75"
                             src={pic.img}
                             alt={pic?.title}
                             title={pic?.title}/>
                    </div>
    ...
    return (
        <>
            <div className={`${!modal ? "modal" : "modal open relative"}`}>
                <img className="rounded-xl" src={holdSrc} alt={holdTitle}/>
                <img className="fixed md:top-10 md:left-10 bottom-7 h-8 cursor-pointer opacity-50"
                     onClick={() => closeImg()}
                     src={closeIcon}
                     alt="fermer close"/>
                <span
                    className="fixed bg-black text-white
                    md:text-3xl md:right-1 md:top-4
                    text-base top-3 px-2 rounded-xl">
                    {holdTitle}
                </span>
            </div>
