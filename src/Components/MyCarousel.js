...
function MyCarousel({personalFav, removeThisFav}) {
    ...
    return (
        ...
        >
            ...
            <div className="transition-all cursor-pointer bg-white bg-opacity-50 hover:bg-[#f9595f] hover:bg-opacity-50 rounded-full py-2 px-3 text-xl mt-3 2xl:mb-16"
                 onClick={() => removeThisFav(personalFav[currentIndex].title)}>
                Supprimer
            </div>
        </div>
    );
    ...
    return (
        <div
            ...
            <div
                ...
                <div
                    className={`w-2/3 Desktop:w-1/3 2K:w-1/4 4K:w-1/3 4K:py-1 mx-auto text-sm text-center z-10 bg-black bg-opacity-50 rounded-xl px-3 font-[Poppins] mt-1 4K:mt-5 4K:text-4xl`}>
                    {image.title}
                </div>
            </div>
